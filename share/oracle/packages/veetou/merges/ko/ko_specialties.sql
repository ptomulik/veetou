    ...
    (
        ...
        (
            ...
            INNER JOIN v2u_ko_page_header_j page_header
                ON (page_header.ko_page_id = sheet_pages.ko_page_id AND
                    page_header.job_uuid = sheets.job_uuid)
            INNER JOIN v2u_ko_headers headers
                ON (headers.id = page_header.ko_header_id AND
                    headers.job_uuid = sheets.job_uuid)
            GROUP BY VALUE(sheets), VALUE(headers), VALUE(preambles)
        ),
        v AS
        (
            SELECT
