CREATE TABLE v2u_ko_przedmioty_cykli_j
    (
          id NUMBER(38)
        , job_uuid RAW(16)
        ...
        , CONSTRAINT v2u_ko_przedmioty_cykli_j_f5
        ...
        , CONSTRAINT v2u_ko_przedmioty_cykli_j_f6
            FOREIGN KEY (prz_kod, cdyd_kod)
            REFERENCES v2u_dz_przedmioty_cykli(prz_kod, cdyd_kod)
    );
/
CREATE SEQUENCE v2u_ko_przedmioty_cykli_j_sq1 START WITH 1;
/
CREATE OR REPLACE TRIGGER v2u_ko_przedmioty_cykli_j_tr1
    BEFORE INSERT ON v2u_ko_przedmioty_cykli_j
    FOR EACH ROW
    WHEN (new.id IS NULL)
    BEGIN
        SELECT v2u_ko_przedmioty_cykli_j_sq1.NEXTVAL INTO :new.id FROM dual;
    END;
