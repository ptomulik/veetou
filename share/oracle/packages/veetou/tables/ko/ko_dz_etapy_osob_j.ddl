CREATE TABLE v2u_ko_dz_etapy_osob_j
    (
        ...
        , CONSTRAINT v2u_dz_etapy_osob_j_f5
        ...
        , CONSTRAINT v2u_dz_etapy_osob_j_f6
            FOREIGN KEY (etpos_id)
            REFERENCES v2u_dz_etapy_osob(id)
    );
/
CREATE SEQUENCE v2u_ko_dz_etapy_osob_j_sq1 START WITH 1;
/
CREATE OR REPLACE TRIGGER v2u_ko_dz_etapy_osob_j_tr1
    BEFORE INSERT ON v2u_ko_dz_etapy_osob_j
    FOR EACH ROW
    WHEN (new.id IS NULL)
    BEGIN
        SELECT v2u_ko_dz_etapy_osob_j_sq1.NEXTVAL INTO :new.id FROM dual;
    END;
