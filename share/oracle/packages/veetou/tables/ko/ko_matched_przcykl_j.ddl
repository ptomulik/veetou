...
/
CREATE OR REPLACE TRIGGER v2u_ko_matched_przcykl_j_tr1
    BEFORE INSERT ON v2u_ko_matched_przcykl_j
    FOR EACH ROW
    WHEN (new.id IS NULL)
    BEGIN
        SELECT v2u_ko_matched_przcykl_j_sq1.NEXTVAL INTO :new.id FROM dual;
    END;
