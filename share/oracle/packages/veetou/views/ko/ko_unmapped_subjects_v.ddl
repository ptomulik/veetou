...
AS
    ...
    INNER JOIN v2u_ko_subject_semesters_j j1
        ON  (
                    j1.subject_id = subjects.id
                AND j1.job_uuid = subjects.job_uuid
            )
    INNER JOIN v2u_ko_specialties specialties
        ON  (
                    specialties.id = j1.specialty_id
    ...
    LEFT JOIN v2u_ko_subject_map_j j2
        ON  (
                    j2.subject_id = subjects.id
                AND j2.specialty_id = j1.specialty_id
                AND j2.semester_id = j1.semester_id
                AND j2.job_uuid = subjects.job_uuid
            )
    LEFT JOIN v2u_subject_map subject_map
        ON  (
                    j2.map_id = subject_map.id
            )
    WHERE subject_map.id IS NULL
          OR j2.matching_score < 2
WITH READ ONLY
;
