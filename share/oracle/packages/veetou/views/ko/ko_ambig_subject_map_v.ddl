...
AS WITH u AS
    (
        ...
        GROUP BY j.subject_id, j.specialty_id, j.semester_id
    ),
    v AS
    (
        SELECT
            V2u_Ko_Ambig_Subject_Map_t(
                  (SELECT VALUE(s) FROM v2u_ko_subjects s WHERE s.id = u.subject_id)
