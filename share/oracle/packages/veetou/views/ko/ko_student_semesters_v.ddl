...
AS WITH u AS (
    ...
    INNER JOIN v2u_ko_student_semesters_j j
        ON (j.student_id = students.id AND
            j.job_uuid = students.job_uuid)
    INNER JOIN v2u_ko_specialties specialties
        ON (specialties.id = j.specialty_id AND
            specialties.job_uuid = students.job_uuid)
    INNER JOIN v2u_ko_semesters semesters
        ON (semesters.id = j.semester_id AND
            semesters.job_uuid = students.job_uuid)
)
SELECT * FROM u u
WITH READ ONLY;
