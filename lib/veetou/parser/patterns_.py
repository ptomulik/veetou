...
_re_word_ = r'(?:%s+)' % _re_letter_ # word made of letters only
_re_worda_ = r'(?:%s%s*)' % (_re_alnum_, _re_alnum_)
...
_rg_student_name_stretched = r'(?P<student_name>(?:%s +)?%s)(?: {1,15}-)?' % (_rg_first_name_stretched, _rg_last_name)
_rg_student_index = r'(?P<student_index>(?:(?:[A-Z]-)?\d{1,6})(?:/PZ)?)'
